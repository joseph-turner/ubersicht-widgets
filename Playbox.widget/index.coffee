...
afterRender: (domEl) ->
  ...
  meta.click ->
    song = meta.find('.song').text()
    artist = meta.find('.artist').text()
    command = 'open ' + '"http://google.com/search?q=' + song + ' ' + artist + '"'
    # console.log command
    self.run command

  art.click ->
    self.run "osascript 'Playbox.widget/lib/spotify-focus.applescript'"
