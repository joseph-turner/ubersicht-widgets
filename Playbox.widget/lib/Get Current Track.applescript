global artistName, songName, albumName, songRating, songDuration, currentPosition, musicapp, apiKey, songMetaFile, mypath, currentCoverURL, isLoved, soundVolume, isAd
set metaToGrab to {"artistName", "songName", "albumName", "songDuration", "currentPosition", "coverURL", "songChanged", "isLoved", "soundVolume", "isAd"}
...
if isMusicPlaying() is true then
	...
	if didSongChange() is true then
		delay 1
		if (artistName = "") then
			writeSongMeta({"albumName" & "##" & albumName, "artistName" & "##" & "Advertisement", "coverURL" & "##" & "NA", "songName" & "##" & songName, "songDuration" & "##" & songDuration, "songChanged" & "##" & true, "isAd" & "##" & true})
			tell application "Spotify"
				set sound volume to 0
			end tell
		else
			tell application "Spotify"
				set sound volume to my readSongMeta({"soundVolume"})
			end tell
			...
			writeSongMeta({"albumName" & "##" & albumName, "artistName" & "##" & artistName, "songName" & "##" & songName, "songDuration" & "##" & songDuration, "isLoved" & "##" & isLoved, "songChanged" & "##" & true, "isAd" & "##" & false})
		end if
	else
...
on getVolume()
	if readSongMeta({"isAd"}) is {"false"} then
		try
			if application "Spotify" is running then
				tell application "Spotify"
					set soundVolume to get sound volume
				end tell
				writeSongMeta({"soundVolume" & "##" & soundVolume})
			end if
		end try
	end if
end getVolume
